...
<template>
    ...
    <main class="grid min-h-screen place-items-center bg-neutral-50 p-4 dark:bg-neutral-950">
        <section class="w-full max-w-md rounded-2xl border bg-white p-6 shadow-sm dark:bg-neutral-900">
            ...
            <p
                v-if="pulseiraLida"
                class="mt-4 flex items-center gap-2 rounded bg-blue-50 p-3 text-sm text-blue-950 dark:bg-blue-950 dark:text-blue-50"
            >
                <ShieldCheck class="h-5 w-5" aria-hidden="true" /> Pulseira reconhecida com segurança.
            </p>
            <p v-else class="mt-4 flex items-start gap-2 rounded bg-amber-50 p-3 text-sm text-amber-950 dark:bg-amber-950 dark:text-amber-50">
                <ScanLine class="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true" /> No primeiro acesso, leia o QR Code da pulseira antes de entrar.
            </p>
