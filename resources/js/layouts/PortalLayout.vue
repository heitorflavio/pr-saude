    ...
    <div class="min-h-screen bg-neutral-50 text-neutral-950 dark:bg-neutral-950 dark:text-neutral-50">
        ...
        <header class="border-b bg-white dark:bg-neutral-900">
            <div class="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3 px-4 py-3">
                ...
                <nav aria-label="Portal do paciente" class="flex flex-wrap items-center gap-4 text-sm">
                    <Link href="/portal">Agora</Link><Link href="/portal/medicamentos">Medicamentos</Link><Link href="/portal/exames">Exames</Link><Link href="/portal/historico">Histórico</Link>
                    <button type="button" class="inline-flex items-center gap-1" @click="sair.post(route('portal.sair'))"><LogOut class="h-4 w-4" aria-hidden="true" /> Sair</button>
                </nav>
            </div>
        </header>
